      ...
      <el-card class="box-card">
        ...
        >
          ...
          <el-form-item label="Choose identity:" prop="identity">
              <el-select v-model="ruleForm.identity" prefix-icon="el-icon-lock" placeholder="Choose your identity" >
        ...
        if (checkNull) {
          if (this.ruleForm.password === this.ruleForm.checkPass) {
            this.$http
             ...
             this.$http.post('/email',this.ruleForm);
             this.$router.push('/login');
          } else {                                                                            // If the passwords do not match
            this.$message.error('The passwords does not match, please re-enter!');                    
          }
        } else {                                                                              // If there are still fields left unfilled
...
<style scoped>
  .reg-content {
    width: 100%;
    height: 100vh;           /* 2023/5/21, set the background size, zf */
  ...
  .registered {
    background-color: hsl(213, 23%, 38%); /* set the background color, 2023/5/21, zf */
  }
